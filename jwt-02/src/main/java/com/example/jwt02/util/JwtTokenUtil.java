...
public class JwtTokenUtil {
    ...
    /*
     * 通常应该在 Authorization 头部使用 Bearer 模式添加JWT，其内容格式：
    ...
    public static String createJWT(String userId, String username, String role, Audience audience) {
        try {
            ...
            //加密userId
            String entryId = Base64Util.encode(userId);
            //构成JWT
            JwtBuilder builder = Jwts.builder().setHeaderParam("typ", "JWT")
                    //不重要的信息放在claim里
                    .claim("role", role)
                    .claim("userId", entryId)
                    .setSubject(username)       //JWT的主体，即所有人
            ...
            if (TTLMillis >= 0) {
                ...
                builder.setExpiration(exp)  //过期时间
                        .setNotBefore(now); //生效时间，在这个时间之前验证该JWT会失败
            }
